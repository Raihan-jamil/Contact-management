...
class _ViewPageState extends State<ViewPage> {
  static String defaultMessage = "Uninformed";

  bool existWhatsapp = false;
  Map? contact;

  late HomeBloc blocHome;
    ...
    ListView content(context, Map snapshot) {
      return ListView(
        children: <Widget>[
          Column(
            children: <Widget>[
              buildHeader(context, snapshot['name'],snapshot['image'] ),
              buildInformation(
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      blocHome.updateFavorite(
                          this.contact!['id'], !snapshot.hasData);
                    },
                  ),
                  IconButton(
                    color: Colors.white,
                    ...
                    onPressed: () {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => EditPage(contact: this.contact!,)),
                      );
                    },
                  ),
                  // IconButton(
      ...
      body: StreamBuilder(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            ...
            contact = snapshot.data as Map;
            blocHome.setFavorite(contact!['favorite'] == 1);
            return content(context, contact!);
          }
        },
      ),
    );
  ...
  void whatsAppOpen(phoneNumber, message) async {
    await FlutterLaunch.launchWhatsapp(phone: phoneNumber, message: message);
  }
  ...
  Container buildHeader(BuildContext context, String name, String image) {
    return Container(
      ...
      child: Column(
        children: <Widget>[
          ...
          image!=null? CircleAvatar(
              radius: 80,
              backgroundImage: MemoryImage(Base64Decoder().convert(image))
          ):CircleAvatar(
            child: Text(
